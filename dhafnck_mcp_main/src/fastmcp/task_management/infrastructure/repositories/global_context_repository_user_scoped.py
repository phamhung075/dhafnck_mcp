...
class GlobalContextRepository(BaseUserScopedRepository):
        ...
        with self.get_db_session() as session:
            ...
            if self.user_id:
                existing = session.query(GlobalContextModel).filter(
                    and_(
                        GlobalContextModel.id == self._normalize_context_id("global_singleton"),
    ...
    def get(self, context_id: str) -> Optional[GlobalContext]:
        ...
        with self.get_db_session() as session:
            ...
            if not db_model:
                logger.debug(f"Global context not found for user {self.user_id}: {original_id}")
            else:
    ...
    def get_user_global_context(self) -> Optional[GlobalContext]:
        """
        Get the user's global context (convenience method).
        
        Returns:
            The user's global context or None if not found
        """
        return self.get("global_singleton")
