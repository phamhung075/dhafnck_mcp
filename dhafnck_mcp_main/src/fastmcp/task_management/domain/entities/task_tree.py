...
class TaskTree:
    ...
    priority: Priority = field(default_factory=Priority.medium)  # Tree-level priority
    status: TaskStatus = field(default_factory=TaskStatus.todo)    # todo, in_progress, blocked, review, testing, done, cancelled, archived
    
    @classmethod
    def create(cls, name: str, description: str, project_id: str) -> 'TaskTree':
        """Create a new TaskTree with a generated UUID"""
        now = datetime.now()
        return cls(
            id=str(uuid.uuid4()),
            name=name,
    ...
    def pause_tree(self) -> None:
        ...
        self.status = TaskStatus.blocked()  # Using blocked as a placeholder for paused
        self.updated_at = datetime.now()
    
    def resume_tree(self) -> None:
        """Resume work on this tree"""
        self.status = TaskStatus.in_progress()  # Using in_progress as a placeholder for active
        self.updated_at = datetime.now()
    ...
    def to_dict(self) -> Dict:
        """Convert TaskTree to dictionary representation"""
        return {
            "id": self.id,
            "name": self.name,
