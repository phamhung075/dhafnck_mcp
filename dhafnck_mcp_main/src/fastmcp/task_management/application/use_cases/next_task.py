...
class NextTaskUseCase:
    ...
    async def execute(self, assignee: Optional[str] = None, project_id: Optional[str] = None, 
        ...
        status_mismatches = await self._validate_task_context_alignment(filtered_tasks)
        ...
        for task in sorted_tasks:
            ...
            if next_subtask:
                ...
                return NextTaskResponse(
                    ...
                    next_item={
                        "type": "subtask",
                        "task": await self._task_to_dict(task, include_context),
                    ...
                    try:
                        ...
                        try:
                            context_result = await asyncio.wait_for(
                                context_service.resolve_full_context(
                                    level="task",
                            ...
                            logger.warning(f"Context resolution timed out for task {task.id.value}")
                ...
                return NextTaskResponse(
                    ...
                    next_item={
                        "type": "task",
                        "task": await self._task_to_dict(task, include_context, user_id, project_id, git_branch_id),
                        "context": self._get_task_context(task, all_tasks)
                    },
    ...
    def _apply_filters(self, tasks: List, assignee: Optional[str], project_id: Optional[str], 
        ...
        if assignee:
            ...
            filtered_tasks = [task for task in filtered_tasks 
                            if task.assignees and isinstance(task.assignees, (list, tuple)) and assignee in task.assignees]
    ...
    async def _task_to_dict(self, task, include_context: bool = False, user_id: str = "default_id", project_id: str = "default_project", git_branch_id: str = "main") -> Dict[str, Any]:
    ...
    async def _validate_task_context_alignment(self, tasks: List) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for task in tasks:
                try:
                    ...
                    context_result = await context_service.resolve_full_context(
                        level="task",
                        context_id=task.id.value
                    )
                    context = context_result.resolved_context if context_result else None
