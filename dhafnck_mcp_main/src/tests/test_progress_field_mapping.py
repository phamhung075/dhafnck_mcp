...
def test_progress_field_mapping():
    ...
    assert task.overall_progress == 0.0, "Initial progress should be 0"
    ...
    assert updated_task.overall_progress == 75.0, "Progress should be updated to 75%"
    
    # Retrieve task to verify persistence
    retrieved_task = repo.get_task(str(task.id))
    assert retrieved_task is not None
    assert retrieved_task.overall_progress == 75.0, "Retrieved task should have correct progress"
    
    # Save task entity with progress
    task.overall_progress = 90.0
    ...
    assert final_task.overall_progress == 90.0, "Saved progress should be persisted"
