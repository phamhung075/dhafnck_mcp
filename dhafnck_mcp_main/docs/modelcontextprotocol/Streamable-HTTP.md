...
## MCP HTTP Header Requirements: Quick Reference

To ensure correct operation and protocol compliance, all MCP clients and servers **MUST** use the following HTTP headers for Streamable HTTP transport:

| Header                | Required? | Value Example                        | When/Why                                      |
|-----------------------|-----------|--------------------------------------|------------------------------------------------|
| Content-Type          | Yes       | application/json                     | All POST requests (body is JSON)               |
| Accept                | Yes       | application/json, text/event-stream  | All requests (support JSON and SSE)            |
| MCP-Protocol-Version  | Yes*      | 2025-06-18                           | All requests after initialization              |
| Mcp-Session-Id        | Yes*      | (from server)                        | All requests after initialization              |
| Origin                | Optional  | (browser origin)                     | For CORS/security, if accessed from browser    |

*\* After `/mcp/initialize`, these are required for all further requests.*

### Best Practices
- **Always** send `Content-Type: application/json` and `Accept: application/json, text/event-stream` on all POSTs.
- **After initialization**, include `Mcp-Session-Id` and `MCP-Protocol-Version` on all requests.
- For browser clients, set a correct `Origin` header.
- If any required header is missing, the server may reject the request (e.g., 400/406/415 errors).
...
headers = {
    ...
    "Mcp-Session-Id": session_id,  # from /mcp/initialize response
}
```

See the table above for when each header is required. For more details, see the full protocol documentation above.
