...
- [ ] Identify all usage of `manage_context` and `manage_hierarchical_context`
...
| Old `manage_context` | Old `manage_hierarchical_context` | New `manage_context` (Unified) |
|---------------------|-----------------------------------|-------------------------------|
...
hierarchical_result = await manage_hierarchical_context(
    action="create",
    level="task",
    context_id=task_id,
...
def test_dual_context_creation():
    ...
    # Test manage_hierarchical_context
    result2 = manage_hierarchical_context(action="create", level="task", context_id="test")
...
2. **Re-enable Legacy Tools**
   ...
   mcp.tool("manage_hierarchical_context_legacy")
   ```
