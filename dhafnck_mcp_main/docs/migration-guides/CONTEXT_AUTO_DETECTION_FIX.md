...
```json
{
  ...
  "hint": "Task management system is unavailable. Provide git_branch_id parameter directly or use manage_hierarchical_context.",
  "immediate_solutions": [
    "Provide git_branch_id parameter: git_branch_id='your-branch-uuid'",
    "Use manage_hierarchical_context instead of manage_context",
    "Check if task management system is properly initialized"
  ],
  "fallback_options": [
    "Provide git_branch_id parameter directly",
    "Use manage_hierarchical_context for direct context operations",
...
manage_hierarchical_context(action="create", level="task", context_id="task-id", data={...})
```
