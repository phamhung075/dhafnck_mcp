...
Instead of tracking client actions, require context updates through tool parameters:

```python
class ContextAwareMCPTools:
    """Wrap existing tools to handle context from parameters"""
    ...
    async def enhanced_manage_task(self, action: str, **kwargs):
        ...
        # Handle context from provided parameters
        elif action == "update":
            task_id = kwargs.get("task_id")
            work_notes = kwargs.get("work_notes")
            progress_made = kwargs.get("progress_made")
            
            # Only update context if AI provided context parameters
            if task_id and (work_notes or progress_made):
                context_update = {
                    "progress": {
                        "completed_actions": [work_notes or progress_made],
                        "last_update": datetime.now().isoformat()
                    }
                }
                await self._context_manager.update_from_params(task_id, context_update)
...
async def manage_task(
    ...
    **kwargs
    ...
    # If work_summary provided, update context from parameters
...
- Update context when AI provides parameters
...
5. **Vision Enrichment**: Server includes vision data in responses
