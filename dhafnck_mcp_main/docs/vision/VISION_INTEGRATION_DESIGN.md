...
This document provides a comprehensive design for integrating the Vision System into the existing dhafnck_mcp_main task management system. The integration leverages the current DDD architecture while adding new capabilities for mandatory context updates, workflow guidance, manual progress tracking with enriched responses, and vision hierarchy enrichment.
...
### 3. Manual Progress Tracking with Enrichment

**Approach**: Required Parameters + Response Enrichment

```python
# AI must manually provide progress
class SubtaskProgressController:
    @tool(description="Update subtask with manual progress")
    def update_subtask_progress(
        self,
        task_id: str,
        subtask_id: str,
        progress_percentage: int,  # AI must provide this
        progress_notes: str        # AI must describe what was done
    ) -> dict:
        # Update subtask and calculate parent progress
        parent = self.task_repo.get(task_id)
        parent.update_progress_from_subtasks()
        
        # Enrich response to remind AI to continue tracking
        return {
            "success": True,
            "parent_progress": parent.overall_progress,
            "reminder": "Remember to update progress after each work step"
        }
```

**Integration Point**: MCP tool parameters and response enrichment
