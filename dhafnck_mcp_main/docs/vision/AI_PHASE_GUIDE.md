...
**Read First**:
- `manual-context-vision-updates.md` → Section: "Block Task Completion Without Updates"
...
### 🤖 Phase 4: Subtask Manual Progress Updates

**Your Task**: Enforce manual progress updates for subtasks through required parameters.

**Read First**:
- `subtask-manual-progress-tracking.md` → **Read entire document**
...
**Key Features**:
1. Require progress_percentage and progress_notes for updates
2. Require completion_summary for subtask completion
3. Calculate parent progress from manual updates
4. Remind AI to update parent context manually
...
async def complete_subtask_with_update(
    ...
    impact_on_parent: str     # REQUIRED
):
    # 1. Complete subtask
    # 2. Calculate parent progress from all subtasks
    # 3. Return reminder to update parent context
```
...
### Pattern 3: Remind AI to Update Parent
```python
# In subtask operations:
if action == "complete":
    parent_progress = calculate_parent_progress(parent_task_id)
    return {
        "success": True,
        "parent_reminder": {
            "progress": parent_progress,
            "action_needed": "Update parent context manually"
        }
    }
```
...
Phase 4 ✓ = Subtasks require manual progress updates
