#!/usr/bin/env python3
"""
🔍 ANALYZE AGENT - Architecture Compliance Analysis with Checkpoint Control
Continuously analyzes architecture compliance and updates workplace.md
"""
...
def wait_for_turn(workplace_path):
    ...
    while not check_my_turn(workplace_path):
...
def analyze_controllers(base_path):
    """Analyze controller layer compliance"""
    violations = []
    files_scanned = 0
    violation_details = []
    
    controller_path = Path(base_path) / 'src/fastmcp/task_management/interface/controllers'
    
    if not controller_path.exists():
        return {
            "files_scanned": 0,
            "violations": 0,
            "details": "- Controller path not found"
        }
    ...
    for controller_file in controller_path.glob('*.py'):
        ...
        try:
            ...
            print(f"  Warning: Could not read {controller_file.name}: {e}")
...
def analyze_factories(base_path):
    """Analyze factory pattern implementation"""
    factory_path = Path(base_path) / 'src/fastmcp/task_management/infrastructure/repositories'
    
    if not factory_path.exists():
        return {
            "files_scanned": 0,
            "violations": 0,
            "details": "- Factory path not found"
        }
    ...
    for factory_file in factory_files:
        try:
            ...
            if uses_central or (has_env_check and has_db_check):
                working_factories += 1
                fixed_details.append(f"✅ {factory_file.name}")
            else:
                broken_details.append(f"❌ {factory_file.name} (Missing env checks)")
        except Exception as e:
            print(f"  Warning: Could not read {factory_file.name}: {e}")
...
def analyze_cache_invalidation(base_path):
    """Analyze cache invalidation implementation"""
    repo_path = Path(base_path) / 'src/fastmcp/task_management/infrastructure/repositories'
    
    if not repo_path.exists():
        return {
            "files_scanned": 0,
            "violations": 0,
            "details": "- Repository path not found"
        }
    
    cached_repos = list(repo_path.glob('cached_*.py'))
    ...
    for repo_file in cached_repos:
        try:
            ...
            if has_invalidation:
                implemented_details.append(f"✅ {repo_file.name}")
            else:
                missing_details.append(f"❌ {repo_file.name}")
        except Exception as e:
            print(f"  Warning: Could not read {repo_file.name}: {e}")
...
def create_workplace_with_checkpoints(analysis_results, workplace_path):
    """Create or update workplace.md with checkpoint control system"""
    ...
    workplace_template = f'''# 📋 ARCHITECTURE COMPLIANCE WORKPLACE
...
# REPLACE WITH:
from ...application.facades import ApplicationFacade
self.facade = ApplicationFacade()
```
...
class RepositoryFactory:
    ...
    def get_repository():
        ...
        if env == 'test':
            ...
            base_repo = SupabaseRepository()
        
        if redis_enabled and env != 'test':
            return CachedRepository(base_repo)
        return base_repo
```
...
def analyze_and_update(base_path, workplace_path):
    """Run analysis and update workplace with results"""
    
    print("🔄 Running architecture analysis...")
    
    # Perform actual analysis
    controller_analysis = analyze_controllers(base_path)
    factory_analysis = analyze_factories(base_path)
    cache_analysis = analyze_cache_invalidation(base_path)
    ...
    # More realistic scoring based on violation severity
    # Controllers: 40 points, Factories: 40 points, Cache: 20 points
    ...
    create_workplace_with_checkpoints(analysis_results, workplace_path)
...
def main():
    ...
    base_path = "/home/daihungpham/__projects__/agentic-project/dhafnck_mcp_main"
    ...
    print("="*60)
    print("🔍 ARCHITECTURE COMPLIANCE ANALYZER")
    print("="*60)
    ...
    while analysis_run <= max_runs:
        ...
        if not check_my_turn(workplace_path):
        ...
        # Run analysis
        compliance_score, total_violations = analyze_and_update(base_path, workplace_path)
        ...
        if compliance_score >= 100:
            ...
            # Update final report
            update_checkpoint(workplace_path, "ANALYZE", "complete")
        ...
        update_checkpoint(workplace_path, "ANALYZE", "complete")
        update_checkpoint(workplace_path, "PLANNER", "active")
        ...
        while True:
            ...
            try:
                ...
                if "**REANALYZE** | active" in workplace or "**ANALYZE** | active" in workplace:
                    ...
                    update_checkpoint(workplace_path, "ANALYZE", "active")
                    update_checkpoint(workplace_path, "PLANNER", "waiting")
                    update_checkpoint(workplace_path, "CODE", "waiting")
                    update_checkpoint(workplace_path, "TEST", "waiting")
                    update_checkpoint(workplace_path, "REVIEW", "waiting")
                    update_checkpoint(workplace_path, "REANALYZE", "waiting")
                    break
