...
context = manage_hierarchical_context(
    action="resolve", 
    level="task", 
    context_id=task_id,
    force_refresh=False
...
global_context = manage_hierarchical_context(action="resolve", level="global", context_id="global_singleton")

# Project-specific rules inherit from global
project_context = manage_hierarchical_context(action="resolve", level="project", context_id=project_id)

# Task context inherits from project and global
task_context = manage_hierarchical_context(action="resolve", level="task", context_id=task_id)
```
...
if fixed:
    ...
    manage_hierarchical_context(action="delegate", level="task", 
                               delegate_to="project", 
...
if insights.is_reusable():
    manage_hierarchical_context(action="delegate", 
...
manage_hierarchical_context(action="delegate", level="project", 
