...
### Via docker-menu.sh (Recommended)
```bash
# Interactive Docker menu with database tools
...
# 3) Supabase + Redis
# P) Performance Mode
# Plus management options for database operations
```
...
2. Select "7) Database Shell" for direct database access
3. Or use the standalone restore script:
   ```bash
   python dhafnck_mcp_main/docker/restore_database.py /path/to/backup.db
   ```

### 2. **Fix "0 tasks" Display Issue**
1. Run `./docker-system/docker-menu.sh`
2. Select the appropriate configuration to start services
3. Use the verification script:
   ```bash
   python dhafnck_mcp_main/docker/database_restore_verification.py /path/to/database.db
   ```
...
- ✅ **docker-menu.sh**: New centralized Docker management system
