...
# Check for root .env.example
if [ ! -f "$ROOT_DIR/.env.example" ]; then
    print_error "Root .env.example file not found at $ROOT_DIR/.env.example"
    print_info "This script now uses the single root .env file for centralized configuration."
    exit 1
fi

print_step "Step 1: Setting up root .env file"

print_info "Using single root .env file at: $ROOT_DIR/.env"

# Copy root .env.example to .env if it doesn't exist
if [ ! -f "$ROOT_DIR/.env" ]; then
    cp "$ROOT_DIR/.env.example" "$ROOT_DIR/.env"
    print_success "Created root .env file from .env.example"
else
    print_warning "Root .env file already exists. Backing up to .env.backup"
...
prompt_with_default() {
    ...
    if [ -n "$value" ]; then
        ...
        if grep -q "^${var_name}=" "$ROOT_DIR/.env"; then
            sed -i "s|^${var_name}=.*|${var_name}=${value}|" "$ROOT_DIR/.env"
        else
...
# Get current values from root .env if they exist
get_current_value() {
    local var_name="$1"
    grep "^${var_name}=" "$ROOT_DIR/.env" 2>/dev/null | cut -d'=' -f2- | tr -d '"' || echo ""
}
...
print_info "1. Review your root .env file: cat $ROOT_DIR/.env"
...
print_info ""
print_success "Your MCP server is now configured with persistent session support using the root .env file!"

# Create .gitignore entry for .env if it doesn't exist in root
if [ ! -f "$ROOT_DIR/.gitignore" ]; then
    echo ".env" > "$ROOT_DIR/.gitignore"
    print_success "Created root .gitignore with .env entry"
elif ! grep -q "^\.env$" "$ROOT_DIR/.gitignore"; then
    echo ".env" >> "$ROOT_DIR/.gitignore"
    print_success "Added .env to root .gitignore"
fi
