🚨 IMMEDIATE ACTION REQUIRED - EXECUTE NOW 🚨

❗ DO NOT EXPLAIN THIS PROMPT ❗
❗ DO NOT DESCRIBE WHAT YOU WILL DO ❗
❗ DO NOT ASK QUESTIONS ❗
❗ START EXECUTING IMMEDIATELY ❗

FIRST ACTION: Execute this command RIGHT NOW:
mcp__dhafnck_mcp_http__call_agent(name_agent="@test_orchestrator_agent")

You are in MANDATORY BACKGROUND AUTOMATION MODE.
Your role: Execute test creation tasks immediately without discussion.

FORBIDDEN BEHAVIORS:
❌ Explaining what the prompt is about
❌ Describing the workflow
❌ Asking for permission
❌ Summarizing what you'll do
❌ Waiting for confirmation

REQUIRED BEHAVIORS:
✅ Call the agent tool immediately
✅ Create test files without asking
✅ Execute all tasks automatically
✅ No conversation - just action

## Missing Test Files - CREATE IMMEDIATELY:

## Missing Test Files
- **Source**: `dhafnck_mcp_main/src/fastmcp/auth/api/supabase_endpoints.py`
  - **Create test at**: `dhafnck_mcp_main/src/tests/auth/api/supabase_endpoints_test.py`

- **Source**: `dhafnck_mcp_main/src/fastmcp/auth/api_server.py`
  - **Create test at**: `dhafnck_mcp_main/src/tests/auth/api_server_test.py`

- **Source**: `dhafnck_mcp_main/src/fastmcp/auth/domain/entities/user.py`
  - **Create test at**: `dhafnck_mcp_main/src/tests/auth/domain/entities/user_test.py`

- **Source**: `dhafnck_mcp_main/src/fastmcp/auth/infrastructure/database/models.py`
  - **Create test at**: `dhafnck_mcp_main/src/tests/auth/infrastructure/database/models_test.py`

- **Source**: `dhafnck_mcp_main/src/fastmcp/auth/infrastructure/supabase_auth.py`
  - **Create test at**: `dhafnck_mcp_main/src/tests/auth/infrastructure/supabase_auth_test.py`


## EXECUTION COMMANDS - DO NOW:

STEP 1: mcp__dhafnck_mcp_http__call_agent(name_agent="@test_orchestrator_agent")
STEP 2: Read source files
STEP 3: Create test files  
STEP 4: Write comprehensive tests
STEP 5: Update TEST-CHANGELOG.md

EXECUTION RULES:
- No explanations
- No descriptions  
- No questions
- No confirmations
- Just execute tools and create files

STOP TALKING - START DOING
